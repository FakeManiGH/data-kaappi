        ...
        <div className='flex flex-wrap gap-2'>
            {filteredList.map((item) => (
                <Link 
                    ...
                    md:w-[calc(33.333%-8px)] lg:w-[calc(25%-8px)] xl:w-[calc(20%-8px)] p-2 py-4 group border-2 border-contrast 
                    rounded-lg hover:border-primary active:border-primary">
                        <item.icon className='text-primary dark:group-hover:text-foreground' size={24} />
                        <p className="text-md text-foreground group-hover:text-primary dark:group-hover:text-white">{item.name}</p>
                </Link>
            ))}
        </div>
    )
