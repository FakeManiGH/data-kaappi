...
function FileGroupSharingForm({ file, setFile, setFiles, setSelectedObjects, groups }) {
    ...
    useEffect(() => {
        ...
        const shareGroups = file.sharing.groups || [];
    ...
    const sortGroups = (fileSharingGroups, allGroups) => {
        const normalizedGroups = new Set(fileSharingGroups.map(group => String(group))); // Normalize IDs
    ...
    const handleChangingFileGroupSharing = async (e) => {
        e.preventDefault();
        if (apiLoading) {
        ...
        try {
            const response = await changeFileGroupSharingStatus(user.id, file.id, groupIDarray);
            if (response.success) {
                showAlert(response.message, 'success');

                // Conditional client side updating
                const updatedFile = { ...file, sharing: { ...file.sharing, groups: groupIDarray }}
                ...
                if (setSelectedObjects) {
                    setSelectedObjects(prevObjects => prevObjects.map(obj => obj.id === updatedFile.id ? updatedFile : obj));
                }
    
                // Re-synchronize selectedGroups and availableGroups
                const { selected, available } = sortGroups(updatedFile.sharing.groups, groups);
                setAvailableGroups(available);
                setSelectedGroups(selected);
    ...
    return (
        <div className='flex flex-col gap-2 mt-2 p-2 bg-gradient-to-r from-contrast to-neutral-400 dark:to-neutral-700 rounded-lg'>
            ...
            {apiLoading ? (
                ...
                <form className='flex flex-col gap-2' onSubmit={handleChangingFileGroupSharing}>
                    <div>
...
export default FileGroupSharingForm