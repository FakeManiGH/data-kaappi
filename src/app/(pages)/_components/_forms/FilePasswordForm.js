...
function FilePasswordForm({ selectedFile, setFile, setFiles, setSelectedObjects, setPasswordPopup }) {
    ...
    const saveFilePassword = async (e) => {
        ...
        try {
            const response = await updateFilePassword(user.id, selectedFile.id, newPassword);
            if (response.success) {
                const updatedFile = {
                    ...selectedFile,
                    passwordProtected: true
                }
                if (setFile) setFile(updatedFile); // Filepage
                if (setFiles) setFiles(prevFiles => prevFiles.map(file => file.id === updatedFile.id ? updatedFile : file)); // Folderpages
    ...
    return (
        <div className="relative mt-2">
            <form className="flex flex-col text-sm" onSubmit={saveFilePassword}> 
                <label htmlFor="password" className="block mt-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                    {selectedFile.passwordProtected ? 'Vaihda ' : 'Aseta '} 
                    salasana
                </label>
