...
import { Eye, EyeOff, LockKeyholeOpen, X } from 'lucide-react'
import { verifyFilePassword } from '@/app/file-requests/files';

function FilePasswordPrompt({ fileID, setFile, setPwdVerified }) {
    ...
    return (
        <div className='flex flex-col gap-2 p-4 justify-center min-h-96 max-w-3xl mx-auto'>
            ...
            <form className="flex flex-col gap-2 text-sm" onSubmit={verifyPassword}>
                ...
                <button 
                    type='submit'
                    className="py-2.5 px-3 rounded-full bg-gradient-to-br from-primary to-blue-800 text-white text-sm hover:to-primary transition-colors
...
export default FilePasswordPrompt