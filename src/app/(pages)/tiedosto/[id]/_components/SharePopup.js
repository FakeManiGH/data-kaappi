...
function SharePopup({ file, setFile, setSharePopup }) {
    ...
    return (
        <div id="overlay" tabIndex="-1" aria-hidden="true" className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
            <div className="flex flex-col max-w-2xl w-full h-[calc(100%-2rem)] max-h-[calc(100%-2rem)] sm:h-fit bg-background dark:bg-contrast rounded-xl overflow-y-auto m-4">
                ...
                <label className={`flex items-center cursor-pointer w-fit p-4 py-2 ${!shared ? 'pb-4' : 'pb-2'}`}>
                    ...
                    />
                    <div className="relative w-11 h-6 bg-gray-400 dark:bg-background peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 
                    ...
                    <div className='flex flex-col gap-2 p-4 py-2'>
                        ...
                        <div className="flex flex-col gap-1 mt-4 pb-2">
                            ...
                            <form className="flex flex-col gap-2">
                                <div className="flex text-navlink rounded-lg overflow-hidden w-full group">
                                    <label htmlFor="email" className='flex bg-contrast group-focus-within:text-primary dark:bg-background items-center text-foreground p-2 px-3 border-r border-contrast'>
                                        <AtSign size={18} />
                                    </label>
                                    <input 
                                        type="email"
                                        name='email' 
                                        className="text-sm w-full p-2.5 outline-none bg-contrast dark:bg-background dark:text-navlink focus:text-foreground" 
                                        placeholder="Anna sähköpostiosoite"
                                    />
                                </div>
                                <button type='submit' className="flex justify-center whitespace-nowrap text-white w-[30%] min-w-fit bg-primary rounded-full p-2 px-4 mt-1 hover:bg-primary/90">
                                    Lähetä sähköposti
                                </button>
                            </form>
                        </div>
